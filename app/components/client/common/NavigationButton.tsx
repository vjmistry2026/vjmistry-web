...
export default function SliderNavButton({ direction, onClick }: SliderNavButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`relative overflow-hidden cursor-pointer border border-[#D9D9D9] group h-[44px] w-[44px] sm:h-[52px] sm:w-[52px] lg:h-[64px] lg:w-[64px] flex items-center justify-center hover:border-primary transition-all duration-300 ${pressed ? "scale-95" : "scale-100"}`}
    >
      {/* Sliding bg */}
      <span
      ...
      <Image
        ...
        width={14}
        height={14}
        className={`pointer-events-none relative z-10 ${arrowRotation[direction]} group-hover:invert group-hover:brightness-0 transition-all duration-300 w-[10px] h-[10px] lg:w-[14px] lg:h-[14px] object-contain`}
      />
    </button>
  );
}